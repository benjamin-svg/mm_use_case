...
products_df["fasttext_embedding"] = products_df["product_combined_text"].apply(
    lambda x: nlp_utils.compute_avg_fasttext(x, fasttext_model, 100)
)
recipes_df["fasttext_embedding"] = recipes_df["recipe_combined_text"].apply(
    lambda x: nlp_utils.compute_avg_fasttext(x, fasttext_model, 100)
)

st.title("mon-marché.fr business case")
display_type = st.sidebar.radio(
    "Type affichage", ["Analyse exploratoire", "Moteur de recherche"]
)
if display_type == "Moteur de recherche":
    ...
    top_products_fasttext, top_recipes_fasttext = nlp_utils.get_relevant_items_fasttext(
        products_df,
        recipes_df,
        query,
        fasttext_model,
        expand_query=bword_expansion,
        threshold=0.2,
    )
    st.caption("Top 20 des produits associés à la recherche")
    st.dataframe(
        top_products_fasttext[["product_title", "product_description"]],
        width=1000,
        hide_index=True,
    )
    st.divider()
    ...
    st.dataframe(
        top_recipes_fasttext[["recipe_name", "recipe_description"]],
        width=1000,
        hide_index=True,
    )
else:
