...
def calculate_diversity(product_df, recipes_df, queries, nlp_model, search_type, top_n=20, threshold=0.2, expand_query=False):
    """
    ...
    for query in queries:
        if search_type == 'bpe':
            top_products, top_recipes = nlp.get_relevant_items_bpe(product_df, recipes_df, query, nlp_model, top_n, threshold)
        elif search_type == 'fasttext':
            top_products, top_recipes = nlp.get_relevant_items_fasttext(product_df, recipes_df, query, nlp_model, expand_query, top_n, threshold)
        elif search_type == 'w2v':
            top_products, top_recipes = nlp.get_relevant_items_w2v(product_df, recipes_df, query, nlp_model, top_n, threshold)
        elif search_type == 'tf_idf':
            top_products, top_recipes = nlp.get_relevant_items_tf_idf(product_df, recipes_df, query, nlp_model, top_n, threshold)
        else:
            raise ValueError(f"Unsupported search type: {search_type}")
            
        product_avg_fasttext.append(np.nanmean(np.array(top_products['fasttext_embedding'].tolist()), axis = 0))
        product_avg_fasttext = [x for x in product_avg_fasttext if str(x) != 'nan']
        
        recipe_avg_fasttext.append(np.nanmean(np.array(top_recipes['fasttext_embedding'].tolist()), axis = 0))
    ...
    if product_avg_fasttext:
        ...
        product_upper_triangle_values = product_similarity_matrix[np.triu_indices(len(product_similarity_matrix), k=1)]
    ...
    if recipe_avg_fasttext:
        ...
        recipe_upper_triangle_values = recipe_similarity_matrix[np.triu_indices(len(recipe_similarity_matrix), k=1)]
...
def calculate_NZR_coverage(product_df, recipes_df, queries, search_type, nlp_model, top_n=20, threshold=0.2, expand_query=False):
    """
    ...
    for query in queries:
        ...
        coverage_recipes = len(list(set(list_of_recipes_ids))) / recipes_df.shape[0] * 100
        coverage_products = len(list(set(list_of_products_ids))) / product_df.shape[0] * 100
   
    return non_zero / len(queries) * 100, queries_with_zero, coverage_recipes, coverage_products

def display_perf_metrics(model, thresholds, non_zero_values, coverage_recipes_values, coverage_products_values, diversity_product_values, diversity_recipes_values):
    """
    ...
    sns.lineplot(data=df.drop('Non Zero Values', axis=1).melt(id_vars=['Threshold'], 
                                                            value_vars=['Coverage Recipes', 'Coverage Products', 'Diversity Products', 'Diversity Recipes'],
                                                            var_name='Metric', 
                                                            value_name='Value'), 
                x='Threshold', y='Value', hue='Metric', palette=palette, linewidth=2.5, ax=ax1)
    ...
    sns.lineplot(data=df, x='Threshold', y='Non Zero Values', color='black', linewidth=2.5, ax=ax2, label='Non Zero Values')
