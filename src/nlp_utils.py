...
def apply_preprocessing(df, columns, operations, stemmer, nlp, stopwords):
    ...
    for col, new_cols in columns.items():
        for operation, new_col in new_cols.items():
            df[new_col] = df[col].apply(lambda x: preprocess_text(x, operation, stemmer, nlp, stopwords))
    return df
...
def preprocess_product_dataframe(product_df):
    ...
    product_df = apply_preprocessing(product_df, columns, operations, stemmer, nlp, stopwords)
    product_df = product_df.rename(columns = {'id': 'article_id', 'title': 'product_title', 'description':'product_description'})
...
def preprocess_receipe_dataframe(receipe_df):
    ...
    receipe_df = apply_preprocessing(receipe_df, columns, operations, stemmer, nlp, stopwords)
    receipe_df = receipe_df.rename(columns = {'name': 'recipe_name', 'description':'recipe_description'})
    
    return receipe_df

def get_relevant_items_tf_idf(product_df, recipe_df, query, vectorizer, top_n=20, threshold=0):
    """
    ...
    product_scores = product_df['product_combined_text'].apply(lambda x: (vectorizer.transform([x]).dot(query_vector.T)).toarray()[0][0])
    recipe_scores = recipe_df['recipe_combined_text'].apply(lambda x: (vectorizer.transform([x]).dot(query_vector.T)).toarray()[0][0])
...
def get_relevant_items_with_embeddings_expansion(product_df, recipe_df, query, w2v_model, vectorizer, top_n=20, expansion_n=3, threshold=0):
    """
    ...
    query = remove_accents(query)
    try:
        expanded_keywords = [word[0] for word in w2v_model.wv.most_similar(query, topn=expansion_n)]
    except:
        expanded_keywords = [] 
...
def get_relevant_items_w2v(product_df, recipe_df, query, w2v_model, top_n=20, threshold=0):
    """
    ...
    product_scores = product_df['w2v_embedding'].apply(lambda x: np.dot(x, query_embedding) / (np.linalg.norm(x) * np.linalg.norm(query_embedding)))
    recipe_scores = recipe_df['w2v_embedding'].apply(lambda x: np.dot(x, query_embedding) / (np.linalg.norm(x) * np.linalg.norm(query_embedding)))
...
def get_relevant_items_fasttext(product_df, recipe_df, query, fasttext_model, expand_query=False, top_n=20, threshold=0):
    """
    ...
    product_scores = product_df['fasttext_embedding'].apply(lambda x: np.dot(x, query_embedding) / (np.linalg.norm(x) * np.linalg.norm(query_embedding)))
    recipe_scores = recipe_df['fasttext_embedding'].apply(lambda x: np.dot(x, query_embedding) / (np.linalg.norm(x) * np.linalg.norm(query_embedding)))
    product_above_thresh = product_scores[product_scores> threshold]
    recipes_above_thresh = recipe_scores[recipe_scores> threshold]
...
def get_relevant_items_bpe(product_df, recipe_df, query, tokenizer, top_n=20, threshold=0):
    """
    ...
    product_scores = product_df['tokenized_text'].apply(lambda x: len(set(query_tokens).intersection(set(x))))
    recipes_scores = recipe_df['tokenized_text'].apply(lambda x: len(set(query_tokens).intersection(set(x))))
